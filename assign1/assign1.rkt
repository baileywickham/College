...
(check-equal? (total-profit 100) 30.0)
...
(define (trick-minute [t : (U Card-Trick Guillotine)]) : Real
  (match t
    [(Card-Trick d v) (* 1 d (expt 2 v))]
    [(Guillotine r g) (if g 20 10)]))
(check-equal? (trick-minute (Card-Trick 2 2)) 8)
(check-equal? (trick-minute (Guillotine 2 #t)) 20)

(struct Quadtratic ([a : Real] [b : Real] [c : Real]) #:transparent)
(struct Linear ([a : Real] [b : Real]) #:transparent)
(define-type Polynomial (U Linear Quadtratic))
; interp takes a polynomial and evaluates it. evaluate function.
(define (interp [p : Polynomial] [x : Real]) : Real
  (match p
  [(Quadtratic a b c) (+ (* a (* x x)) (* b x) c)]
  [(Linear a b) (+ (* a x) b)]))
(check-equal? (interp (Linear 2 2) 3) 8)
(check-equal? (interp (Quadtratic -1 2 2) 3) -1)

;TODO ask about linear poly derivative
; Derivative takes a polynomial and returns the polynomial derivative.
(define (derivative [p : Polynomial]) : Polynomial
  (match p
    [(Linear a b) (Linear 0 a) ]
    [(Quadtratic a b c) (Linear (* 2 a) b)]))
(check-equal? (derivative (Linear 2 2)) (Linear 0 2))
(check-equal? (derivative (Quadtratic 2 2 1)) (Linear 4 2))
