...
final class Entity implements Animatable{
    ...
    {
        return new Activity(world, this);
    }
    ...
    {
        Entity fullTarget
                = world.findNearest(this.getPosition(), EntityKind.BLACKSMITH);

        if (fullTarget != null  &&
                this.moveToFull(world, fullTarget, eventSchedule))
        {
            this.transformFull(world, eventSchedule);
        }
        else
        {
    ...
    executeMinerNotFullActivity(WorldModel world,
                                EventSchedule eventSchedule)
    {
        Entity notFullTarget = world.findNearest(this.getPosition(),
                EntityKind.ORE);

        if (notFullTarget == null ||
                !this.moveToNotFull(world, notFullTarget, eventSchedule) ||
                !this.transformNotFull(world, eventSchedule))
        {
            eventSchedule.scheduleEvent(this,
                    this.createActivityAction(world),
                    this.getActionPeriod());
                       ...
                       EventSchedule eventSchedule)
    {
        ...
        Entity blob = Entity.createOreBlob(pos, this.getActionPeriod() / 4,
                50 + rand.nextInt(100));
                           ...
                           EventSchedule eventSchedule)
    {
        Entity blobTarget = world.findNearest(
                this.getPosition(), EntityKind.VEIN);
        ...
        if (blobTarget != null)
        {
            ...
            if (this.moveToOreBlob(world, blobTarget, eventSchedule))
            {
