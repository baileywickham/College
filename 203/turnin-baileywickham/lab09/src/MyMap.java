...
class MyMap<K,V> {
    private List<List<MyMapEntry<K,V>>> data;
    private int size;
    MyMap(int buckets) {
        data = new ArrayList<>(buckets);
        this.size = buckets;
        while (data.size() < buckets) data.add(new ArrayList<>());
    }

    public V get(K key) {
        for (MyMapEntry<K,V> l : data.get(key.hashCode() % size)) {
            if (l.value.equals(key) ){
                return l.value;
            }
    ...
    public void put(K key, V value) {
        MyMapEntry<K,V> append = new MyMapEntry<K,V>(key,value); // awkwark
        List<MyMapEntry<K,V>> entry = data.get(key.hashCode() % size);
        entry.add(append);

    }
    public List<MyMapEntry<K,V>> getEntries() {
        List<MyMapEntry<K,V>> ret = new ArrayList<>();
        for (List<MyMapEntry<K,V>> l : data) {
            if (!l.isEmpty()) {
                ret.addAll(l);
            }
        }
